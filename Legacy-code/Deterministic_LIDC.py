...
from Utilis import seg_score, segmentation_scores, CustomDataset_LIDC, calculate_cm
...
if __name__ == '__main__':
    ...
    it = iter(testloader)
    next(it)
    images, true_image, annots, imagename = next(it)
    # annots_list = [images[0,0,:,:], true_image[0,0,:,:]]
    annots_list = [true_image[0,0,:,:]]
    for idx in range(annots.shape[4]):
        annot = annots[0,0,:,:,idx]
        annots_list.append(annot)

    plt.imsave('./figures/lidc-scan2', images[0,0,:,:], cmap='gray')
    # # call model:
    ...
    # model_name = 'PLEASEWORK' + '_width' + str(width) + \
